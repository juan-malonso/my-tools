...
import type { Allocation, ItemDate } from '@/models';

import { getSpanDates } from './dateUtils';
...
/**
 * Finds the next available (unoccupied) cell in the grid row.
 *
 * @param {ItemDate[]} dates - The full array of available dates.
 * @param {Allocation[]} allocations - The list of all current task allocations.
 * @param {number} dateOffset - The starting index to search from.
 * @returns {ItemDate | undefined} The next free date cell, or undefined if none.
 */
