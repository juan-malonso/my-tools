...
import { type Task, type Utils, type Allocation, type Member, type Module } from '@/models';
...
export const AllocationSettings: React.FC<{
...
}> = ({ task, tasks, allocations, modules, members, defaultBadgeKey }) => {
  const memberOptions = members.values.map((m) => ({ value: m.id, label: m.name }));
  const moduleOptions = modules.values.map((m) => ({ value: m.id, label: m.name }));
  ...
  return (
    <div key={task.id} className=" bg-slate-900/50 p-4 rounded-lg border border-slate-700/50">
      <div className="grid grid-cols-2 grid-cols-[auto_250px] gap-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          {taskAllocations.map((allocation, index) => {
            const member = members.values.find((m) => m.id === allocation.memberId);
            const memberHours = member
              ? allocation.span * (member.schedule.find((_, i) => i < 5) ?? 0)
              : 0;

            const isCollision = hasCollision(allocation);

            return (
              <div
                key={index}
                className="space-y-2 p-2 bg-slate-800 rounded-lg border border-slate-700/50"
              >
                <div className="flex justify-between items-center gap-2">
                  <Select
                    ...
                    onChange={(e) => {
                      allocations.set({ ...allocation, memberId: e.target.value });
                    }}
                    size="sm"
                    className="w-1/2"
                    placeholder="---"
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      allocations.set({ ...allocation, moduleId: e.target.value });
                    }}
                    size="sm"
                    className="w-1/4"
                    placeholder="---"
                ...
                <div className="flex justify-between items-center gap-2">
                  <Input
                    type="date"
                    value={allocation.iniDate}
                    className={`w-1/2 ${isCollision ? '!border-red-500 !text-red-500' : ''}`}
                    onChange={(e) => {
                      allocations.set({ ...allocation, iniDate: e.target.value });
                    }}
                    size="sm"
                  />
                  <Input
                    type="number"
                    value={allocation.span}
                    className="w-1/4"
                    onChange={(e) => {
                      allocations.set({ ...allocation, span: parseInt(e.target.value, 10) });
                    }}
                    size="sm"
                    min={1}
                  />
